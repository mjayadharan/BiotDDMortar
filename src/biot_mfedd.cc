...
//			fe (FE_RaviartThomas<dim>(degree), dim,
//				FE_DGQ<dim>(degree-1), dim,
//				FE_DGQ<dim>(degree-1), 0.5*dim*(dim-1),
...
// Assembling system matrix for couple Biot with non-differentiated elasticity equation
template <int dim>
		void MixedBiotProblemDD<dim>::assemble_system_non_diff()
		{
			TimerOutput::Scope t(computing_timer, "Assemble system");
			system_matrix = 0;
			//system_rhs_bar = 0;
...
//                    Vector<double >pressure_values(boundary_values_flow.begin(),boundary_values_flow.end());
        ...
        //Note that we are using the non-differentiated elasticity equation in this case
//        solve_star();
        {
        	assemble_system_non_diff();
        	A_direct_non_diff.initialize(system_matrix);
        ...
        for (;cell!=endc;++cell)
        {
            for (unsigned int face_n=0;
                ...
                if (cell->at_boundary(face_n) && cell->face(face_n)->boundary_id() != 0)
                {
                    ...
                    for (unsigned int d_i=0; d_i<dim; ++d_i)
                    {
                        fe_face_values[stresses[d_i]].get_function_values (solution_star, solution_values_mech[d_i]);
                        fe_face_values[stresses[d_i]].get_function_values (interface_fe_function, interface_values_mech[d_i]);
                    }
                    ...
                    {
                        for (unsigned int d_i=0; d_i<dim; ++d_i)
                            return_vector[0] += fabs(fe_face_values.normal_vector(q) * solution_values_mech[d_i][q] *
                                        (displacement_values[q][d_i] - fe_face_values.normal_vector(q) * interface_values_mech[d_i][q] * get_normal_direction(cell->face(face_n)->boundary_id()-1)) *
                                        fe_face_values.JxW(q));
                        return_vector[1] += fabs(fe_face_values.normal_vector(q) * solution_values_flow[q] *
                                                                (pressure_values[q] - fe_face_values.normal_vector(q) * interface_values_flux[q] * get_normal_direction(cell->face(face_n)->boundary_id()-1)) *
                                                                fe_face_values.JxW(q));
                    }
                }
        }
...
//        return_vector[1]+= return_vector[0];
    ...
    template <int dim>
            std::vector<double> MixedBiotProblemDD<dim>::compute_interface_error_l2()
            {
                    ...
                    for (unsigned int face_n=0;
                        ...
                        if (cell->at_boundary(face_n) && cell->face(face_n)->boundary_id() != 0)
                        {
                            ...
                            for (unsigned int q=0; q<n_face_q_points; ++q)
                                for (unsigned int i=0; i<dofs_per_cell; ++i)
                                {
                                    return_vector[1] += pow(exact_values[q][dim*dim + dim + 0.5*dim*(dim-1)+ dim]
															- interface_values_flux[q] *fe_face_values.normal_vector(q)
                                    						*get_normal_direction(cell->face(face_n)->boundary_id()-1), 2)
                                                      *fe_face_values.JxW(q);
//
////                                    for (unsigned int d_i=0; d_i<dim; ++d_i)
////                                        sigma[d_i] = fe_face_values[stresses[d_i]].value (i, q);
//
    ...
    void MixedBiotProblemDD<dim>::compute_errors (const unsigned int cycle)
    {
      ...
      err.linf_l2_errors[0] = std::max(err.linf_l2_errors[0],sqrt(u_l2_error)/sqrt(u_l2_norm));
      double total_time = prm.time_step * prm.num_time_steps;
      {
		...
		u_l2_norm+=u_hd_norm;
		err.velocity_stress_linf_div_errors[0] = std::max(err.velocity_stress_linf_div_errors[0],sqrt(u_hd_error)/sqrt(u_hd_norm));
      }
...
//      err.linf_l2_errors[2] = std::max(err.linf_l2_errors[2],sqrt(s_l2_error)/sqrt(s_l2_norm));
      ...
      err.linf_l2_errors[2] = std::max(err.linf_l2_errors[2],sqrt(s_l2_error)/sqrt(s_l2_norm)); //L8-L2 for stress
      ...
      s_l2_norm+= s_hd_norm;
      err.velocity_stress_linf_div_errors[1] = std::max(err.velocity_stress_linf_div_errors[1],sqrt(s_hd_error)/sqrt(s_hd_norm));
//      err.linf_l2_errors[2] = std::max(err.linf_l2_errors[2],sqrt(s_l2_error)/sqrt(s_l2_norm));
...
//      //old way of calculating interace error usuing discrete norm
//      double l_int_error_elast, l_int_norm_elast;
//      double l_int_error_darcy, l_int_norm_darcy;
...
////            }
//            err.linf_l2_errors[5] =std::max(err.linf_l2_errors[5], l_int_error_elast/l_int_norm_elast) ;
//            err.linf_l2_errors[6] = std::max(err.linf_l2_errors[6], l_int_error_darcy/l_int_norm_darcy);
//
////            err.linf_l2_norms[5] = std::max(err.linf_l2_norms[5], l_int_norm_elast);
////			err.linf_l2_norms[6] = std::max(err.linf_l2_norms[6], l_int_norm_darcy);
//        }

      // FInding (u,p) - (lambda_u,lambda_p) error in L2 norm
      if (mortar_flag)
      {
    	   // ||u-lambda_u||_L2
    		  std::vector<double> tmp_err_vect(2,0);

    		  tmp_err_vect = compute_interface_error_l2();

    	      err.linf_l2_errors[5] = std::max(err.linf_l2_errors[5], sqrt(tmp_err_vect[0])/sqrt(d_l2_norm));
    	   // ||p - lambda_p||
    	      err.linf_l2_errors[6] = std::max(err.linf_l2_errors[6], sqrt(tmp_err_vect[1])/sqrt(p_l2_norm));
      ...
      if(std::fabs(prm.time-total_time) < 1.0e-12)
      {
        ...
        double send_buf_num[13] = {err.velocity_stress_linf_div_errors[0],
                                   err.linf_l2_errors[0],
                                   err.l2_l2_errors[1],
                                   ...
                                   err.linf_l2_errors[4],
								   err.linf_l2_errors[5],
								   err.linf_l2_errors[6]};
...
//          if (i!=1 && i != 4 && i != 7 && i != 0 && i != 8 && i != 11 && i != 12 && i!=10)
        	if (i==2 || i==5 || i==6 || i==9 )
        	{
...
//        convergence_table.add_value("Stress,L8-Hdiv", recv_buf_num[7]);
        ...
        convergence_table.add_value("Rotat,Linf-L2", recv_buf_num[10]);
...
//        convergence_table.set_scientific("Pressure,L2-L2mid", true);
        ...
        convergence_table.set_precision("Stress,L8-L2", 2);
        convergence_table.set_precision("DivStress,L8-L2", 2);
//        convergence_table.set_precision("Stress,L2-Hdiv", 3);
//        convergence_table.set_precision("Stress,L8-Hdiv", 2);
        convergence_table.set_precision("Displ,L8-L2", 2);
//        convergence_table.set_precision("Displ,L2-L2mid", 3);
        convergence_table.set_precision("Rotat,Linf-L2", 2);

        convergence_table.set_scientific("Stress,L8-L2", true);
        convergence_table.set_scientific("DivStress,L8-L2", true);
...
//        convergence_table.set_tex_caption("Pressure,L2-L2mid", "$ \\|Qp - p_h\\|_{L^2(L^2)} $");
        ...
        convergence_table.set_tex_caption("DivStress,L8-L2", "$ \\|\\nabla\\cdot(\\sigma - \\sigma_h)\\|_{L^{\\infty}(L^2)} $");
...
//        convergence_table.set_tex_caption("Displ,L2-L2mid", "$ \\|Q\\bbeta - \\bbeta_h\\|_{L^{\\infty}(L^2)} $");
        convergence_table.set_tex_caption("Rotat,Linf-L2", "$ \\|\\gamma - \\gamma_h\\|_{L^{\\infty}(L^\\infty)} $");

        /*
         * Convergence table rate definitions
         */
...
//        convergence_table.evaluate_convergence_rates("Pressure,L2-L2mid", ConvergenceTable::reduction_rate_log2);
        ...
        convergence_table.evaluate_convergence_rates("DivStress,L8-L2", ConvergenceTable::reduction_rate_log2);
...
//        convergence_table.evaluate_convergence_rates("Displ,L2-L2mid", ConvergenceTable::reduction_rate_log2);
        ...
        if (mortar_flag)
        {
          convergence_table.set_precision("Lambda,Elast", 2);
          ...
          convergence_table.set_precision("Lambda,Darcy", 2);
          ...
          if(split_flag==0){
        	  convergence_table.set_precision("Lambda,Biot", 2);
